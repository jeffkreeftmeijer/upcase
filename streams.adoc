...
This example includes the `Read` and `Write` traits to ensure they're in scope, as `Stdin` and `Stdout` use these in their implementations to provide the `Read::read_to_string()` and `Write::write_all()` methods.
